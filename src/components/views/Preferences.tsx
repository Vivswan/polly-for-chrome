...
function getVoiceOptions(session: SessionStorage, language: string, engine: string): VoiceOption[] {
	...
	const voicesInLanguage =
		session.voices?.filter(
			(voice) => voice.languageCodes.includes(language) && voice.supportedEngines.includes(engine)
		) || [];
	...
	const voiceNames = voicesInLanguage.map(({ name: value, ssmlGender }) => {
		...
		const title = value;
		const description = ssmlGender.toLowerCase().charAt(0).toUpperCase() + ssmlGender.toLowerCase().slice(1);
...
function getEngineOptions(session: SessionStorage, language: string): EngineOption[] {
	...
	const voicesInLanguage = session.voices?.filter((voice) => voice.languageCodes.includes(language)) || [];

	const engines = new Set(voicesInLanguage.map((voice) => voice.supportedEngines).flat());
